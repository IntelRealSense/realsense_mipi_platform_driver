...
int main(int argc, char** argv) {
    ...
    char* end;
    int laserPowerValue = strtol(argv[2], &end, 10);

    if (laserPowerValue > 360 || laserPowerValue < 0) {
        cout << "laser power value should be between 0-360" << endl;
        return 0;
    }
    ...
    // step is 30
    if (abs(currManualLaserCtrl.value - laserPowerValue) > 30) {
